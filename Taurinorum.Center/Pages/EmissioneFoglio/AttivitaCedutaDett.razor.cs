...
{
    public partial class AttivitaCedutaDett
    {
        protected override RenderFragment BaseContent => (builder) => base.BuildRenderTree(builder);
        AttivitaCedutaDTO Dettaglio = new AttivitaCedutaDTO();
        ...
        protected override async void Salva()
        {
            try
            {
                ...
                var ret = await restService.AttivitàCeduta_SetAsync(Dettaglio);
        ...
        {
            Pagina = EPagina.AttivitaCedutaDett;
            try
            {
                if (ID != null && ID != 0)
                {
                    var result = await restService.Attività_GetByIdAsync(ID);
                    if (result != null)
                    {
                        Dettaglio = (AttivitaCedutaDTO)result;
                        InModifica = true;
                    }
                    else
                        VisualizzaMessaggio(new Allert() { Valore = 0, Messaggio = "Attività non trovata" });
                    await JsRuntime.InvokeVoidAsync("Errore", "Attività non trovata");
                }
            }
            catch (Exception ex)
            {
