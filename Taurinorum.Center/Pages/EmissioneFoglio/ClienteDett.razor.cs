    ...
    public partial class ClienteDett
    {
        ...
        protected override async void Salva()
        {
            try
            {
                ...
                var ret = await restService.Cliente_SetAsync(Dettaglio);
        ...
        {
            Pagina = EPagina.ClienteDett;
            try
            {
                if (ID != null && ID != 0)
                {
                    var result = await restService.Cliente_GetByIdAsync(ID);
                    if (result != null)
                        Dettaglio = (ClienteDTO)result;
                    else
                        VisualizzaMessaggio(new Allert() { Valore = 0, Messaggio = "Cliente non trovata" });
                    await JsRuntime.InvokeVoidAsync("Errore", "Cliente non trovata");
                }
            }
            catch (Exception ex)
            {
