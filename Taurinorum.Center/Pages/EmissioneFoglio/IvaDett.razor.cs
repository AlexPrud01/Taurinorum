    ...
    public partial class IvaDett
    {
        ...
        protected override async void Salva()
        {
            try
            {
                ...
                var ret = await restService.Iva_SetAsync(Dettaglio);
        ...
        {
            Pagina = EPagina.IvaDett;
            try
            {
                if (ID != null && ID != 0)
                {
                    var result = await restService.Iva_GetByIdAsync(ID);
                    if (result != null)
                        Dettaglio = (IvaDTO)result;
                    else
                        VisualizzaMessaggio(new Allert() { Valore = 0, Messaggio = "Iva non trovata" });
                    await JsRuntime.InvokeVoidAsync("Errore", "Iva non trovata");
                }
            }
            catch (Exception ex)
            {
