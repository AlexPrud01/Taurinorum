...
{
    public partial class ModalitaPagamentoDett
    {
        protected override RenderFragment BaseContent => (builder) => base.BuildRenderTree(builder);
        ModalitaPagamentoDTO Dettaglio = new ModalitaPagamentoDTO();
        ...
        protected override async void Salva()
        {
            try
            {
                ...
                var ret = await restService.ModalitaPagamento_SetAsync(Dettaglio);
        ...
        {
            Pagina = EPagina.ModalitaPagamentoDett;
            try
            {
                if (ID != null && ID != 0)
                {
                    var result = await restService.ModalitaPagamento_GetByIdAsync(ID);
                    if (result != null)
                        Dettaglio = (ModalitaPagamentoDTO)result;
                    else
                        VisualizzaMessaggio(new Allert() { Valore = 0, Messaggio = "ModalitaPagamento non trovata" });
                    await JsRuntime.InvokeVoidAsync("Errore", "ModalitaPagamento non trovata");
                }
            }
            catch (Exception ex)
            {
